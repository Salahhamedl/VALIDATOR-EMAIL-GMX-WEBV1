...
foreach ($lists as $list) {
    ...
    //API
    $api = $APIs . "/validator/WEB_DE_GMX_DE/?list=$list&proxy=$Proxies&proxyAuth=$proxy_Auth&type_proxy=$type_proxy&apikey=$apikey";
    //CURL
...
$rollingCurl->setCallback(function (\RollingCurl\Request $request, \RollingCurl\RollingCurl $rollingCurl) use (&$results) {
    global $listname, $no, $total, $live, $die, $chkAG, $unknown, $limit, $rto;
    $no++;
    ...
    if (strpos($x, '"status":"success"')) {
        ...
        $email_status = $js['data']['info']['email_status'];
        $message = $js['data']['info']['message'];
...
echo " CHECK AGAIN      : " . $chkAG . PHP_EOL;
